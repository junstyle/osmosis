...
Command.prototype.start = function (context, data) {
    ...
    return callback.call(this, context, data, function next_f(c, d, index) {
        if (calledNext === true) {
            // If `next` is called more than once,
            // then we need to clone the data
            self.next.start(c, d.clone().setSortIndex(index).ref())
        } else {
            calledNext = true
            self.next.start(c, d.setSortIndex(index))
        }
    }, function done_f(err) {
        data.unref()
...
Command.prototype.request = function (method, context, href, params, callback, sortIndex, onLoaded) {
    var self = this,
    ...
    Command.prototype[name] = (function () {
        ...
        process.nextTick(function () {
            if (self.next !== undefined) {
                ...
                func.apply(self.prev || self, args)
            } else {
