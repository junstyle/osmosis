 ...
 * @param {number} [delay] - delay for requesting next page
 * @instance
 */
...
function Paginate(context, data, next, done) {
    ...
    method = document.location.url.method || 'get';
    ...
    self.request(method, node, url, params, function (document) {
        ...
        if (self.delay == undefined) {
            if (self.prev.name == 'delay') {
                self.delay = self.prev.delay;
            }
        }
        setTimeout(() => {
...
module.exports.paginate = function (selector, limit, delay) {
